...
public class DressController {
    ...
    ResponseEntity<List<DressListPageDTO>> getcommpanyTop5Dresses(String commpanyName){
        try {
            List<DressListPageDTO> dresses = dressReadOnlyService.getcommpanyTop5Dresses(commpanyName);
            System.out.println("Retrieved dresses:"+dresses);
            return ResponseEntity.ok(dresses);
        }catch (Exception e){
            e.printStackTrace();
    ...
    @GetMapping(value = "/newdress", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
    ...
    @GetMapping(value = "/newdressTest", produces = MediaType.MULTIPART_FORM_DATA_VALUE, consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    @Operation(summary = "새 드레스를 등록 합니다 테스트 입니다. 임의의 업체 아이디를 받습니다..")
    ResponseEntity<String> dressNewRegistrationTEST(
            String testAccountId,
            @ModelAttribute DressNewRegistrationDTO dto,
            @ModelAttribute MultipartFile front,

            @ModelAttribute MultipartFile side,
            @ModelAttribute MultipartFile back
    ) {
        dto = dto.toBuilder()
                .front(front)
                .side(side)
